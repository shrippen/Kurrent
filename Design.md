...
Prosa-Referenz für UI- und UX-Entscheidungen. Laufzeit-Tokens: `plasmoid/com.github.shrippen.kurrent/contents/ui/Design.qml`. Akzente: `contents/ui/colors.js`. Projektübergreifende Designsprache (Farbpalette, Typografie, Icon-Stil, Landing-Page-Template): **[DesignDefault](https://github.com/shrippen/shrippen.github.io)**.
...
### Matrix-Ansichten (Swimlanes und Projektplan)

Beide teilen sich `components/MatrixGrid.qml`: **ein** `GridLayout` in einer `Flickable`, Zeilenhöhen ergeben sich aus der höchsten Zelle. Spaltenkopf-Zeile und Zeilenkopf-Spalte sind **angepinnt**, indem ihre Items per `Translate` mit `contentX`/`contentY` mitlaufen (kein zweites, synchronisiertes Flickable). Ecke oben links: Button „Zum aktuellen Zeitraum springen“. Scroll wie Kanban: `Kirigami.WheelHandler`, `QQC2.ScrollBar` (keine `ThinScrollBar`), Mittelklick-Pan in beide Achsen. Spalten wachsen bei Platzüberschuss (`stretchColumns`), nie unter die Token-Breite. Maße in `Design.qml` (`swimlaneColumnWidth`, `planColumnWidth(bucket)`, `matrixHeaderHeight`, `matrixRowHeader{Min,Max}Width`, `planCellHeight`, `matrix*Opacity`). Zeilenkopfbreite folgt dem breitesten Label (`TextMetrics`, zwischen Min und Max).

- **Virtualisierung (Swimlanes).** Eine Zelle ist zunächst nur ein leeres `Item`; ihren Inhalt (Tönung, Gitterlinien, Karten, Maus- und Drop-Flächen) baut sie erst, wenn sie bis auf **eine Bildschirmbreite** an den sichtbaren Bereich heranreicht, und behält ihn dann. Ohne das kostete ein Achsenwechsel mit vielen Zeilen mehrere hundert Millisekunden Blockade, weil alle Zellen sofort vollständig gebaut wurden (gemessen: ~200 Zellen erzeugt, ~20 davon tatsächlich sichtbar). Ungebaute Zellen reservieren eine aus der Kartenzahl geschätzte Höhe, damit die Scroll-Länge plausibel bleibt.
- **Gedrosselte Scroll-Position.** Die Sichtbarkeitsprüfung der Zellen bindet an `MatrixGrid.viewX`/`viewY`, **nicht** an `contentX`/`contentY`: sonst würde pro gescrolltem Pixel je eine Binding-Auswertung **pro Zelle** laufen. `viewX`/`viewY` springen sofort mit, sobald der Blick sich um eine halbe Bildschirmbreite bewegt hat (der Ladepuffer ist eine ganze), sonst gesammelt nach kurzer Ruhe. Die **angepinnten Köpfe** folgen weiterhin dem ungedrosselten Offset — sie dürfen nicht nachlaufen.
- **Karten-Deckel.** Eine Swimlane-Zelle zeigt höchstens `Design.swimlaneMaxCardsPerCell` (8) Karten, der Rest wird zu „+N weitere Aufgaben“ (Klick = Drilldown wie die übrige Zelle). Das ist zugleich Lesbarkeit und Budget: eine Zelle mit 26 gestapelten Karten ist weder lesbar noch billig, und ohne Deckel bleibt die Bauzeit proportional zur Aufgabenzahl statt zur Bildschirmfläche.

- **Zeitachse** (Backend `TaskLogic::buildMatrix`): Bucket **Tag / Woche / Monat**. Schlüssel: ISO-Datum, ISO-Jahr-Woche `YYYY-Www` (ISO-Jahr, nicht Kalenderjahr — sonst landet der 31. Dezember in der falschen Woche), `YYYY-MM`. Davor eine **Überfällig**-Sammelspalte (Bucket < aktueller), dahinter **Später** (jenseits des Horizonts) und **Ohne Datum**. Die aktuelle Periode ist hervorgehoben (Akzent-Tönung, 2-px-Linie, Titel in Akzentfarbe); im Tages-Bucket sind Wochenenden getönt. Beschriftungen entstehen in QML (`matrixlabels.js`, `i18n`/`Qt.locale()`), das Backend liefert nur Schlüssel plus `timeStart`/`timeEnd`.
- **Horizont** = Perioden **nach** der aktuellen. Swimlanes: Default *automatisch* (14 Tage / 8 Wochen / 6 Monate, KCM „Swimlane horizon“ 0–52), leere Perioden bis zum Horizont bleiben als Drop-Ziele. Projektplan: 0 = alles (hartes Cap 366 Tage / 104 Wochen / 60 Monate), nur bis zur letzten belegten Periode. Nichts wird still abgeschnitten — was jenseits liegt, steht in **Später**.
- **Kopfzeilen-Steuerung** (`matrixOptionsButton`, nur in diesen beiden Modi, Icon `view-grid`): Zeilenachse (nur Swimlanes), Spalten-Bucket, Vorausschau, beim Plan zusätzlich „Show undated / completed“. Schreibt dieselben `kurrentrc`-Keys wie die KCM „Views“.
- **Drilldown → Liste + Filter-Chip.** Klick auf Zelle, Zeilenkopf oder Spaltenkopf wechselt in den **Listenmodus** und filtert nur die Liste (`setMatrixDrilldown(source, lane, time, label)`, leerer Schlüssel = Wildcard); der Filter wirkt **nur im Listenmodus** und wird beim Verlassen der Liste verworfen — die Matrix wird nie aus einer gefilterten Menge gebaut (früher schrumpfte der Projektplan nach einem Zellklick auf genau diese Zelle). Der Wechsel läuft über `requestMainPaneMode` → `plasmoidRoot.setMainPaneMode` (persistiert korrekt; ein direktes Schreiben von `backend.mainPaneMode` würde von `applyMainPaneMode()` zurückgesetzt). In der Kopfzeile erscheint neben den Filtern ein **Chip** (z. B. „Arbeit · KW 38“): Klick = zurück zur Matrix, ✕ = Filter entfernen. Der Drilldown endet automatisch beim Wechsel des Modus (weg von Liste) oder der Sidebar-View.
- **Undo:** siehe Swimlanes (Drop) — Matrix-Drop ist **ein** Undo-Schritt (`Edit`).
- **Leerer Zustand:** `Kirigami.PlaceholderMessage` (Icon, Text, Erklärung), Ladezustand „Loading tasks…“.
- Header/Zeilenkopf: Tooltip „Show N tasks in the list“, Zeiger-Cursor nur bei N > 0; Hover aus, solange `interactionsSuspended` (Full-Editor offen).

### Swimlanes

- **Zeilen** = Achse (Projekt, Label, Priorität oder Wurzel-Parent), **Spalten** = Zeit (Default **Woche**, umschaltbar auf Tag/Monat) aus `DUE`, ersatzweise `DTSTART`. Zeilen in Sidebar-artiger Ordnung: Projekte alphabetisch, Labels alphabetisch (+ „No label“ zuletzt), Priorität Hoch → Keine, Parent alphabetisch nach Titel. Projekt-/Label-/Prioritätszeilen sind **feste Vokabulare** und bleiben auch leer sichtbar (Drop-Ziele), Parent-Zeilen sind datengetrieben. Zeilenkopf: Achsen-Icon in Sidebar-Farbe, Name, Anzahl.
- **Zellen enthalten kompakte Karten** (`KanbanCard` mit `compact: true`: Chips, Titel max. 3 Zeilen, kein Beschreibungstext, keine Aktionsleiste; Klick öffnet den Full-Editor). Reihenfolge in der Zelle = aktuelle Sortierung. Kein `busy-day`-Streifen mehr: Spaltenköpfe zeigen die Summe je Periode, die Heatmap deckt den Tages-Überblick ab.
- **Drag & Drop** wie Kanban: die Karte selbst wird gezogen (Schwelle `dragThreshold`, Platzhalter in der Quellzelle, **kein** Proxy-Bild — `useTaskDragProxy` ist in Kanban und Swimlanes aus), Zielzelle mit Rahmen und Tönung, Hinweis „Drop here“. Ein Drop schreibt in **einem** Schritt (`moveTaskToMatrixCell`, ein Undo):
  - **Zeilenachse:** Projekt → Verschieben in den Kalender (Inbox ist kein Ziel); Label → Ziel-Label wird **erstes** Label (bzw. „No label“ leert die Labels); Priorität → Band; Parent → `RELATED-TO` (Zyklen und fremde Kalender werden mit Fehlermeldung abgelehnt).
  - **Zeitachse:** neue Fälligkeit im Ziel-Bucket. Mit vorhandener Fälligkeit bleiben Uhrzeit, Ganztägigkeit und **Wochentag** (Woche) bzw. **Tag im Monat** (Monat, auf Monatsende geklemmt) erhalten; ohne Fälligkeit: Bucket-Beginn (in der laufenden Periode „heute“). Liegt das Ergebnis in der Vergangenheit der laufenden Periode, wird auf heute geklemmt. „Ohne Datum“ entfernt die Fälligkeit.
  - **Überfällig** und **Später** sind **keine** Drop-Ziele (blenden beim Ziehen aus); erledigte Karten sind nicht ziehbar.
  - Drop in die eigene Zelle ist ein No-op.

### Projektplan

- Matrix: Zeilen = Projekte (Inbox zuerst, sonst alphabetisch; nur Projekte mit Aufgaben), Spalten = Zeit-Bucket (Default Woche). Zelle = **Anzahl** Aufgaben, kein Workload (Legende unter der Matrix; `DURATION`/`KURRENT/ESTIMATE` bleibt nach 1.0).
- **Zellfarbe** = relative Intensität (`count / max`), nicht absolut: die vollste Zelle der Matrix ist die dunkelste. Die Überfällig-Spalte ist rot getönt. Unter der Zahl: **roter Punkt** = mindestens eine überfällige Aufgabe in der (nicht-überfälligen) Zelle, **Flagge** = mindestens eine Aufgabe mit hoher Priorität. Tooltip: „N tasks · X overdue · Y high priority · Z completed“.
- Klick auf Zelle/Zeilen-/Spaltenkopf → Liste + Filter-Chip (siehe Matrix-Ansichten). „Erledigte anzeigen“ (Kopfzeilen-Menü/KCM) wirkt auch auf den Drilldown, damit Zahl und Liste übereinstimmen.
- Kein Drag & Drop (eine Zelle ist eine Zahl, keine Karte).
- **Icon:** `view-pim-tasks`; Ansichtsmodus-Liste in `FullView.viewModeOptions`.
