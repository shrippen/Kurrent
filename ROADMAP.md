...
Out of scope for 1.0: Kanban, calendar grid, Markdown notes, attachments, a login of its own, a theme engine besides Plasma, or free-form pixel padding.
...
- Overdue as its own view or a section inside Today.
...
- `VALARM` in the editor, Plasma notifications when due.
...
- One-step undo for delete / complete / move.
- KCM: row chips, click vs double-click, editor defaults, default reminder.
...
- Keyboard: focus search, new task, complete, delete, full editor, view switching.
...
- Show/hide and reorder individual views (including Overdue after 0.2).
...
- Per-chip toggles: date, labels, priority, percent complete, location, recurring icon.
- Relative dates vs locale date; time on the row on/off.
- Overdue emphasis via the due chip, not a second palette.
- Subtask indent in grid units; default collapsed/expanded.
- Section headers when a `KURRENT/LIST` property is set.

### Editor

- Always inline, always full, or compact first with a full-editor button.
- Which inline fields (description, due, priority, labels).
- Fold status / secrecy / location / percent / recurrence in the full editor.
...
- Search, new, complete, delete, editor, views 1–9 listed in the KCM.
...
6. Tests: recurrence complete, alarm round-trip, quick-add parser, overdue view, `SharedSettings` round-trip.

## After 1.0

- Kanban, week calendar, Markdown, attachments, comments
...
Settings keys and empty KCM pages in parallel with 0.2 packaging. Task/editor settings with 0.3. Appearance, sidebar, panel, and colors with 0.4. KRunner last.
